...
ggplot(data = dt, aes(SalePrice)) + 
  geom_histogram(fill = "#003700", color = "#FFFFF0", bins = 18) + 
...
ggplot(data = dt, aes(x = `Lot Area`, y = SalePrice)) + 
  geom_point(size = .7, color = "#003700") +
...
dt[, avgSP := mean(SalePrice), by = Neighborhood]       %>%
  ...
  ggplot(aes(x = Neighborhood, y = SalePrice)) + 
    geom_boxplot(color = "#003700", fill = "#FFFFF0") +
...
model <- lm(SalePrice ~ ., data = dt)

# View the summary of the model
sum_mod <- summary(model) 
paste("Multiple R-squared:", sum_mod$r.squared,	
      "Adjusted R-squared:", sum_mod$adj.r.squared
)
